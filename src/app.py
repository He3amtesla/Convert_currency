...
if total_number > 0 and base_currency and target_currency:
    exchange_rate, last_uptated = get_exchange_money(base_currency, target_currency)
    
    if exchange_rate:
        convert_amount = convert_currency(exchange_rate, total_number)
        st.success(f"✅Exchange Rate:{convert_amount:.4f} (Last uptated(±3h): {last_uptated})") #:.4f نشان دادن با 4 رقمت اعشاری
